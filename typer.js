...
// TODO: THE_LAYOUT var is defined in layout files. 
// This should be defined in the Typer object in the future.
var THE_LAYOUT;
...
function doCheck() {
    ...
    var juri = Typer.appUrl + "/mod/mootyper/atchk.php?status=2&attemptid="
            + rpAttId + "&mistakes=" + Typer.mistakes + "&hits="
...
function keyPressed(e) {
    ...
    if (keychar === Typer.currentChar
        ...
        Typer.currentPos++;
        return true;
    } else if (keychar === ' ' && !Typer.countMistypedSpaces) { 
        ...
        if ((!Typer.continuousType && !Typer.countMistypedSpaces)
                || (!Typer.continuousType && Typer.countMistypedSpaces)) { 
            // If not set for continuous typing, wait for correct letter.
            return false;
        } else if (Typer.currentPos < Typer.fullText.length - 1) { 
